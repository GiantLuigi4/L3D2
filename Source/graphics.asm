_clear_fbuf:
	...
	mov	rdi, qword[rel FRAMEBUFFER]	;rdi is now start addr for fbuf
	mov	esi, dword[rel FRAMEBUFFER+8]	;and rsi is length of allocated space
	lea	rsi, [rsi+rdi]	;now rsi is addr+len so addr end
	add	rdi, HEADER_LEN	;add header length to rdi to skip header
	mov	r8w, word[rel var.sky_colour]	;move sky colour word into r8
	mov	r9b, byte[rel var.sky_colour+2]	;and last byte into r9
.loop_clear:
	;----------------------------------------
...
_clear_zbuf:
	...
	mov	rax, qword[rel zbuf_reset]	;move the reset qword into rax
	mov	rdi, qword[rel DEPTHBUFFER]	;and depth buffer start addr here
	mov	esi, dword[rel DEPTHBUFFER+8]	;then load buffer size
...
_draw_line:
	...
	mov	di, word[rel line.x1]	;load x1 into rdi
	sub	di, word[rel line.x0]	;subtract x0 to get difference
...
.x0_smaller:
	...
	mov	si, word[rel line.y1]	;load y1 into rsi
	sub	si, word[rel line.y0]	;then get difference
...
.y0_larger:
	...
	movzx	rax, word[rel line.x0]	;start line at x0
	movzx	rbx, word[rel line.y0]	;and y0
.loop_draw:
	;----------------------------------------
	;DRAW LINE AND CHECK IF DONE
	call	_draw_pixel	;draw a pixel at position
	cmp	ax, word[rel line.x1]	;check if x is at end
	jnz	.skip_ret	;if no, skip further checks
	cmp	bx, word[rel line.y1]	;otherwise, check if y at end
...
_draw_mesh:
	...
	mov	r15, qword[rel FRAMEBUFFER]	;use this addr to draw
...
.found_faces:
	...
	test	byte[rel status.wireframe], 0b10000000	;check if wireframe mode
...
_draw_textures:
	...
	test	byte[rel status.backfaces], 0b10000000	;test if culling off
...
lea_offset rcx, [rel objbuf]
	...
	movaps	xmm9, [rel simd_one]	;move {1, 1, 1, 1} into xmm9
	...
	movss	dword[rel scratchpad+32], xmm0	;store d00 as first denom val to multiply
	movss	dword[rel barycentric.v+8], xmm0	;and third mul val for computing v and w
	;----------------------------------------
	;D01
	...
	movss	dword[rel scratchpad+36], xmm0	;and then save as second val for denom here
	movss	dword[rel scratchpad+52], xmm0	;and also fourth value
	movss	dword[rel barycentric.v+4], xmm0	;then second mul val here
	movss	dword[rel barycentric.v+12], xmm0	;and fourth mul val
	;----------------------------------------
	;D11
	movaps	xmm0, xmm4	;load xmm4 into xmm0 for d11
	pmulld	xmm0, xmm0	;multiply by itself
	phaddd	xmm0, xmm0	;add x and y
	movss	dword[rel scratchpad+48], xmm0	;then store as third val to mul
	movss	dword[rel barycentric.v], xmm0	;and first val to mul here
	;----------------------------------------
	;COMPUTE BARYCENTRIC DENOMINATOR
	movaps	xmm0, [rel scratchpad+32]	;load {d00 d01 0 0}
	movaps	xmm1, [rel scratchpad+48]	;load {d11 d01 0 0}
	pmulld	xmm0, xmm1	;multiply to get {d00*d11 d01*d01}
	phsubd	xmm0, xmm0	;subtract d01*d01 from d00*d11
	movss	dword[rel barycentric.denom], xmm0	;and store as denom
	mov	edx, dword[rel barycentric.denom]	;move val into rdx (sign extend)
	mov	qword[rel barycentric.denom], rdx	;then store back
	fild	qword[rel barycentric.denom]	;then load onto FPU as integer
	fst	qword[rel barycentric.denom]	;then save back as float
	emms	;clear FPU stack	
	;----------------------------------------
	;GET TRIANGLE BOUNDING BOX
	; TODO: this will not work
	mov	rdx, qword[rax]	;move point A XY into rdx
	mov	qword[rel scratchpad], rdx	;and save to scratchpad
	mov	rdx, qword[rbx]	;move point B XY into rdx
	mov	qword[rel scratchpad+8], rdx	;and save to scratchpad too
	movaps	xmm0, [rel scratchpad]	;now xmm0 = {X0 Y0 X1 Y1}
	movaps	xmm1, [rcx]	;xmm1 = {X2 Y2 Z2 1}
	...
	movaps	xmm2, [rel simd_zero]	;now load zero set into xmm2
	...
	movaps	xmm2, [rel simd_screen]	;move this set in {sX sY sX sY}
	...
	movaps	[rel scratchpad], xmm0	;save this to scratchpad
	;----------------------------------------
	;INITIALISE REGS FOR BOX
	mov	r8d, dword[rel scratchpad+4]	;r8 = start Y
	mov	r9d, dword[rel scratchpad]	;r9 = start X
	mov	r10d, dword[rel scratchpad+12]	;r10 = end Y
	mov	r11d, dword[rel scratchpad+8]	;r11 = end X
	;----------------------------------------
...
.even_row:
	imul	rdi, qword[rel term_size.xb]	;multiply Y by row size in bytes
...
.loop_draw:
	...
	movss	xmm6, dword[rel simd_one]	;move a 1 into xmm6
...
.next_row:
	...
	add	rdx, qword[rel term_size.xzb]	;add on row to zbuf addr
	...
	mov	r9d, dword[rel scratchpad]	;restore X counter
...
_draw_wireframe:
	...
	test	byte[rel status.backfaces], 0b10000000	;test if culling disabled
...
.skip_cull:
	...
	%macro	line_coords	0
        ...
        ; TODO: this will not work
	    add rax, [rel objbuf]
		mov	r8d, dword[rax]	;move x0 into r8
		mov	word[rel line.x0], r8w	;then r8 into line x0
		mov	r8d, dword[rax+4]	;move y0 into r8
		mov	word[rel line.y0], r8w	;then r8 into line y0

	    add rbx, [rel objbuf]
		mov	r8d, dword[rbx]	;move x1 into r8
		mov	word[rel line.x1], r8w	;then r8 into line x1
		mov	r8d, dword[rbx+4]	;move y1 into r8
		mov	word[rel line.y1], r8w	;then r8 into line y1
...
_draw_pixel:
	...
	movzx	r9, word[rel term_size+6]	;now get the width vals into here
	movzx	r10, word[rel term_size+4]	;and height values here
...
.even_row:
	imul	rbx, qword[rel term_size.xb]	;multiply the halved number row width
...
_draw_pixel_raw:
	;----------------------------------------
	;PERFORM DEPTH CHECK
	movss	dword[rel scratchpad+64], xmm6	;move correct w into scratchpad
	fld	dword[rel scratchpad+64]	;load correct w
...
_init_screen:
	...
	mov	rdi, qword[rel FRAMEBUFFER]	;move in framebuffer addr
	mov	ecx, dword[rel FRAMEBUFFER+8]	;move in framebuffer length
	add	rcx, rdi	;now add together to get framebuffer end addr
	;----------------------------------------
	;WRITE FRAMEBUFFER HEADER AND LOAD ESCAPES
	mov	r8d, dword[rel esc_home]	;load the home escape into r8
	mov	dword[rdi], r8d	;write home escape to header area
	add	rdi, HEADER_LEN	;go to body section now
	mov	r8, qword[rel unit_template]	;load 8 bytes of unit template
	mov	r9, qword[rel unit_template+8]
	mov	r10, qword[rel unit_template+16]
	mov	r11w, word[rel unit_template+24]	;then remaining 2 bytes here
.loop_write:
	;----------------------------------------
